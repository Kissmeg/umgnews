    ...
    <div className='code animation-scale ease-in-out transition-all'>
        ...
        <div>
            <div className="grid grid-cols-6 grid-rows-1 gap-4 ">
                <div className="col-span-2 border p-4 rounded-xl shadow-xl h-[200px]">
                <p className='text-2xl mb-1 text-center'>Total Articles</p>  
                    <div className='flex items-center w-full justify-center size-24 rounded-full'>
                        <div className='flex justify-center border-2 bg-blue-100 size-24 items-center rounded-full hover:scale-110 ease-in-out transition-all cursor-pointer' onClick={()=>setSelectedMenu('readArticle')}>
                            <p className='text-4xl'>{data.length}</p>
                        </div>
                    </div>
                </div>

                <div className="col-span-2 col-start-3 border p-4 rounded-xl shadow-xl h-[200px]">
                <p className='text-2xl mb-1 text-center'>Total Articles By Category</p>
                    <div className='text-xs code grid grid-cols-5 mt-2 gap-4'>
                        <Link to='https://www.umgnews.com/politics' target='_blank'>
                            <div className='lg:w-[80px] lg:h-[50px] bg-blue-100 w-fit flex flex-col justify-center items-center p-2 rounded-xl hover:scale-110 ease-in-out transition-all cursor-pointer'> 
                                <p>Politics</p>
                                <p>{politicsCount}</p>
                            </div>
                        </Link>
                ...
                <div className="col-span-2 col-start-5 border p-4 rounded-xl shadow-xl h-[200px]">
                    ...
                    <div className='overflow-hidden' key={index}>
                        <div className='flex'>
                            <div className='w-[150px] h-[150px] overflow-hidden rounded-lg'>
                                <Link to={`https://www.umgnews.com/news/${item.headingslug}/${item.id}`} target='_blank'><img className='w-[150px] h-[150px] hover:scale-110 object-cover ease-in-out transition-all' src={item.image[0]} alt="" /></Link>
                            </div>
                            <div>
                                <Link to={`https://www.umgnews.com/news/${item.headingslug}/${item.id}`} target='_blank' ><p className='text-xs ml-4'>{item.heading.slice(0,20)}...</p></Link>
                            </div>
                        </div>
                        <div>
                        </div>
